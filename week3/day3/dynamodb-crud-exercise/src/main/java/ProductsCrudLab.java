...
public class ProductsCrudLab {
    ...
    public static void createProductTable() {
        ...
        try {
            CreateTableRequest request = CreateTableRequest.builder()
                    .attributeDefinitions(AttributeDefinition.builder().attributeName("category").attributeType(ScalarAttributeType.S).build(), AttributeDefinition.builder().attributeName("sku").attributeType(ScalarAttributeType.S).build())
                    .keySchema(KeySchemaElement.builder().attributeName("category").keyType(KeyType.HASH).build(), KeySchemaElement.builder().attributeName("sku").keyType(KeyType.RANGE).build())
            ...
            while (!active) {
                describeTableResponse = ddb.describeTable(DescribeTableRequest.builder().tableName(TABLE_NAME).build());
                active = describeTableResponse.table().tableStatus() == TableStatus.ACTIVE;
                if (!active) {
                    Thread.sleep(500);
                }
            }
            ...
            System.err.println("[CREATE TABLE] Error creating table: " + e.getMessage());
        } catch (InterruptedException e) {
            Thread.currentThread().interrupt();
            System.err.println("[CREATE TABLE] Interrupted while waiting for table to become active.");
        }
    }
    ...
    public static void putProduct(String category, String sku, String productName,
        ...
        try {
            ...
            items.put("category", AttributeValue.builder().s(category).build());
            items.put("sku", AttributeValue.builder().s(sku).build());
            items.put("product_name", AttributeValue.builder().s(productName).build());
            items.put("price", AttributeValue.builder().n(String.valueOf(price)).build());
            items.put("brand", AttributeValue.builder().s(brand).build());
            items.put("in_stock", AttributeValue.builder().bool(inStock).build());
            items.put("units_available", AttributeValue.builder().n(String.valueOf(unitsAvailable)).build());
            ...
            if (e.getMessage().contains("ConditionalCheckFailed")) {
                System.err.println("[PUT] Error: " + sku + " already exists in " + category + ". Insert rejected.");
            } else {
                System.err.println("[PUT] Error inserting item: " + e.getMessage());
            }
        }
    }
